...
describe Takky::UploadWorker do
  ...
  describe "#perform", integration: true do
    let(:attachment) {
      TakkyModelAttachment.create!(extension: "jpg", style: "source")
    }
    let(:fonzie) { File.join("spec/fixtures/images/fonzie.jpg").to_s }
    let(:s3_path) { "img/t/#{attachment.id}/c72a84ebcd.jpg" }

    it "sets the digest" do
      expect(subject).to receive(:upload).with(fonzie, s3_path)

      subject.perform("TakkyModelAttachment", attachment.id, fonzie)
    ...
    xit "deletes the previous attachment" do
      ...
      subject.perform("TakkyModelAttachment", attachment.id, slide)
      attachment.reload
  ...
  describe "#post_process" do
    let(:attachment) { TakkyModelAttachment.new }
    ...
    it "runs if the post_process_with key is set" do
      ...
      expect(TakkySupport::FakeProcessor).to receive(:perform_async).
        with("TakkyModelAttachment", 1, url, args)
    ...
    it "works with a single post processor" do
      ...
      expect(TakkySupport::FakeProcessor).to receive(:perform_async).
        with("TakkyModelAttachment", 1, attachment.url, {}) { "jid" }
  ...
  describe "#delete_previous_version" do
    it "deletes the previous version if present" do
      ...
      img = instance_double("TakkyModelAttachment", path: path, uploaded?: true)
    ...
    it "does not delete if previous version is not present" do
      img = instance_double("TakkyModelAttachment", path: nil, uploaded?: false)
